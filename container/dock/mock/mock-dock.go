...
type mockFactory struct {
	...
	instances map[string]docker.Container
...
func MockFactory() Container {
	return &mockFactory{
		...
		instances: make(map[string]docker.Container),
	}
}
...
func (mock *mockContainer) Create(configFile, template string, templateArgs ...string) error {
    ...
    // Or: mock.state.Pid != 0
	if mock.state.Running || mock.state.Ghost {
		return fmt.Errorf("container is already created")
	}
	//mock.state = golxc.StateStopped
    //FIXME public ? mock.state.Running = false
...
func (mock *mockContainer) Wait(states ...docker.State) error {
	return nil
}

// Info returns the status and the process id of the container.
//func (mock *mockContainer) Info() (golxc.State, int, error) {
func (mock *mockContainer) Info() (docker.State, int, error) {
	//pid := -1
    // I think docker is considering 0 as non-existant pid (see State.setStopped())
	pid := 0
...
func (mock *mockFactory) String() string {
	return fmt.Sprintf("mock dock factory")
}

//func (mock *mockFactory) New(name string) golxc.Container {
func (mock *mockFactory) New(name string) docker.Container {
...
func (mock *mockFactory) List() (result []docker.Container, err error) {
