...
// dockBridgeName is the name of the network interface that the local provider
...
type localEnviron struct {
	...
	containerManager      dock.ContainerManager
}
...
func (env *localEnviron) SetConfig(cfg *config.Config) error {
	...
	env.containerManager = dock.NewContainerManager(
		dock.ManagerConfig{
...
func (env *localEnviron) bootstrapAddressAndStorage(cfg *config.Config) error {
	...
	logger.Debugf("found %q as address for %q", bridgeAddress, dockBridgeName)
...
func (env *localEnviron) StartInstance(
	...
	network := dock.DefaultNetworkConfig()
...
func (env *localEnviron) AllInstances() (instances []instance.Instance, err error) {
	...
	dockInstances, err := env.containerManager.ListContainers()
	if err != nil {
		return nil, err
	}
	for _, inst := range dockInstances {
...
func (env *localEnviron) findBridgeAddress() (string, error) {
	return getAddressForInterface(dockBridgeName)
}
