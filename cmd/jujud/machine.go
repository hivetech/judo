...
func (a *MachineAgent) StateWorker() (worker.Worker, error) {
	...
	if providerType != provider.Local && m.ContainerType() != instance.LXC {
        // -------------------------------------------------------------------
        //if providerType == instance.Hive {
        if m.ContainerType() == instance.DOCK {
            workerName := fmt.Sprintf("%s-provisioner", provisioner.DOCK)
            runner.StartWorker(workerName, func() (worker.Worker, error) {
                return provisioner.NewProvisioner(provisioner.DOCK, st, a.MachineId, dataDir), nil
            })
            // ---------------------------------------------------------------
        } else {
