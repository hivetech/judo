...
func ExerciseHostedServicesAPI(api *gwacl.ManagementAPI) {
    ...
    checkError(err)
    preciseImage, err := images.GetLatestUbuntuImage(release, location)
    checkError(err)
    sourceImageName := preciseImage.Name
    ...
    linuxConfigurationSet := gwacl.NewLinuxProvisioningConfigurationSet(hostname, username, password, "TEST_USER_DATA", "false")
    ...
    role := gwacl.NewRole("ExtraSmall", roleName, []gwacl.ConfigurationSet{*linuxConfigurationSet, *networkConfigurationSet}, []gwacl.OSVirtualHardDisk{*vhd})
    machineName := makeRandomIdentifier("gwaclmachine", 20)
    deployment := gwacl.NewDeploymentForCreateVMDeployment(machineName, "Staging", machineName, []gwacl.Role{*role}, "")
